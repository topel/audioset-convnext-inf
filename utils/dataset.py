...
def pack_waveforms_to_hdf5(args):
    ...
    # Read csv file
    meta_dict = read_metadata(csv_path, audios_dir, classes_num, id_to_ix)
    ...
    motif = re.compile(r"_\d+_\d+.flac$")
    ...
    with h5py.File(waveforms_hdf5_path, 'w') as hf:
        ...
        for n in range(audios_num):
            ...
            if os.path.isfile(audio_path):
                ...
                # (audio, _) = librosa.core.load(audio_path, sr=sample_rate, mono=True, res_type="soxr_hq")
                # (audio, _) = librosa.core.load(audio_path, sr=None, mono=True)
                # print("librosa", audio[:3])
                # 05/12/2022: sf read is so much faster than librosa load:
                (audio, orig_sr) = sf_read(audio_path)
                if audio.ndim > 1:
                    audio = np.mean(audio, axis=1)
                if orig_sr != sample_rate:
                    # audio = resample(audio, orig_sr=orig_sr, target_sr=sample_rate, res_type="soxr_hq")    
                    audio = resample(audio, orig_sr=orig_sr, target_sr=sample_rate, res_type="kaiser_best")    
                # print("sf", audio[:3])
                # print("audio shape", audio.shape)
                
                audio = pad_or_truncate(audio, clip_samples)
                # if n==0:
                #     sf.write("toto_%s.wav"%meta_dict['audio_name'][n], audio, sample_rate, subtype='PCM_16')
                #     print("toto_%s.wav"%meta_dict['audio_name'][n])
                #     print(audio.shape, len(audio), np.sum(audio))

                short_audio_name = re.sub(motif, "", meta_dict['audio_name'][n])
                # print("short_audio_name", short_audio_name)
                hf['audio_name'][n] = short_audio_name.encode()
                # print("hf['audio_name'][n]", hf['audio_name'][n])
