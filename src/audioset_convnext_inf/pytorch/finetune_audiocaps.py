...
from audioset_convnext_inf.pytorch.pytorch_utils import move_data_to_device
from audioset_convnext_inf.utils.aac_datasets_utils import BasicCollate
from audioset_convnext_inf.utils.audiocaps import AudioCaps
...
train_loader = DataLoader(
    train_dataset, batch_size=bs_train, collate_fn=BasicCollate(with_tags=True)
)
val_loader = DataLoader(
    val_dataset, batch_size=bs_test, collate_fn=BasicCollate(with_tags=True)
)
test_loader = DataLoader(
    test_dataset, batch_size=bs_test, collate_fn=BasicCollate(with_tags=True)
)
...
def evaluate_model(model, dataloader, subset):
    ...
    average_precision = metrics.average_precision_score(
        all_tags, all_outputs, average=None
    )
...
for epoch in range(1, 20):
    ...
    torch.save(
        model.state_dict(),
        fpath="audiocaps/model_epoch_%04d_mAP_%.4f.pt" % (epoch, mAP),
    )
