...
def tag_2_onehot(tag_list_one_item: List, num_classes: int = 527):
    ...
    return torch.scatter(
        torch.zeros(1, num_classes),
        1,
        src=torch.ones(1, num_classes),
        index=torch.unsqueeze(torch.as_tensor(tag_list_one_item), 0),
    )
...
class BasicCollate:
    ...
    def __init__(
        self,
        audio_fill_value: float = 0.0,
        with_tags: bool = False,
        num_tag_classes: int = 527,
    ) -> None:
        super().__init__()
    ...
    def __call__(self, batch: List[Dict[str, Any]]) -> Dict:
        ...
        if self.with_tags:
            tags_batch = torch.cat(
                [tag_2_onehot(item["tags"], self.num_tag_classes) for item in batch]
            )
        ...
        if not all(are_tensors):
            raise TypeError(
                f"Invalid audio type in {self.__class__.__name__}. (found are_tensors={are_tensors})"
            )
        ...
        if not all(are_paddable):
            raise TypeError(
                f"Invalid audio shapes in {self.__class__.__name__}. (found are_paddable={are_paddable})"
            )
